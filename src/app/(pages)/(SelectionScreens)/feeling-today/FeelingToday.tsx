...
import { disconnectSocket } from '@/app/redux/users/userChat';
import { setNotificationCount } from '@/app/redux/users/notificationSlice';
interface Condition {
  name: string;
  color: string;
  icon: JSX.Element;
...
const FeelingToday = () => {
  ...
  const getProfileData = async () => {
    ...
    try {
      ...
      socket?.on("notificationCount", (data) => {
        console.log('OnFeelingsCOunt', data)
  ...
  const continueSelection = () => {
    socket?.on("notificationCount", (data) => {
      console.log('FeeligTOdayCOuntAPI222', data)
      // setNotificationCount(data?.count)

    });
    if (selectedItems != "") {

      router.push("/make-better-day")
    } else {
  ...
  //   socket?.on("notificationCount", (data) => {
  //     console.log('feelingOutSIdeData', data)

  //   });
  // }, [socket])
  const dispatch = useDispatch()
  useEffect(() => {
    console.log('socketFEELING', socket)
    if (socket?.connected) {
      getProfileData();
    }
    socket?.on("notificationCount", (data) => {
      console.log('FeelingTodayRuns', data);
  ...
  return (
    <div className='bg-primary min-h-svh flex py-[30px] flex-col justify-center items-center max-[424px]:px-0 px-5 relative'>
      <div>
        <div>
        ...
        <div className="grid-container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 md:py-[50px]  py-[25px]">
          {conditions.map((condition, index) => (
            ...
            >
              ...
              <div className={`${condition.color} rounded-[20px] w-[329px] !max-sm:w-full h-[78px] flex justify-center items-center ${selectedItems.includes(condition.name) ? 'border-2 border-black' : ''}`}>
