...
const HomeRightContent = () => {
    ...
    return (
        <div className="pt-[40px] ">
            ...
            <div className=" bg-[#D2E4C4] pt-7  w-full rounded-xl">
                <Tabs className="MoodTabs" defaultIndex={1}>
                    ...
                    <TabPanel >
                        <div className='p-7'>
                        <div className="topic-list bg-primary p-5 rounded-lg" style={{ maxHeight: '660px', overflowY: 'auto' }}>
                            {topics?.map((topic, index) => (
                                <div className={`flex mb-6  justify-between items-center ${index !== topics?.length - 1 ? 'border-b border-gray-300 pb-4' : ''}`} key={topic?.attributes?.id}>
                                    <div className="topic-item flex items-center gap-2">
                                        <img src={topic?.attributes?.image} alt="" className="cursor-pointer text-center rounded-[10px] h-10 w-10" />
                                        {/* <Image
                                            width={40}
                                            height={40}
                    ...
                    <TabPanel >
                        <div className="p-7">
                        <div className="challenge-listy bg-white p-7 rounded-lg" style={{ maxHeight: '660px', overflowY: 'auto' }}>
                            {challenges?.map((challenge, index) => (
                                <div className={`flex p-5 font-[500] rounded-xl mb-3 text-center justify-center items-center  ${index !== challenges?.length - 1 ? 'border-b border-gray- pb-4' : ''}`} key={challenge?.id}
