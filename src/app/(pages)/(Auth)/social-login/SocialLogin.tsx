...
import "../../../globals.css"
import { SendUserDataSchema } from "@/app/schema/SendUserDataSchema";
...
function SocialLogin() {
  ...
  const { values, handleBlur, handleChange, handleSubmit, errors, touched } =
    useFormik({
      initialValues,
      validationSchema: SendUserDataSchema,
        ...
        fetch(API_ENDPOINTS.SOCIALLOGIN, requestOptions)
          ...
          .then((result: any) => {
            if (
              ...
              const NickName = Encrytion(
                result.message.data.attributes?.nickName
              );
              const Gender = Encrytion(result.message.data.attributes?.gender);
              const FirstName = Encrytion(
                result.message.data.attributes?.firstName
              );
              const LastName = Encrytion(
                result.message.data.attributes?.lastName
              );
              const Email = Encrytion(result.message.data.attributes?.email);
              const Role = Encrytion(result.message.data.attributes?.role);
              const Token = Encrytion(result.message.authToken);
        ...
        <form onSubmit={handleSubmit} className=" " noValidate>
          ...
          <div className="pt-[48px]">
            <div className="sm:flex flex-column  items-center flex-wrap justify-center sm:gap-5 max-sm:gap-y-6">
              <p className="font-montserrat font-[700] text-center pe-5">
                Gender
              </p>
          ...
          {selectedGenderState ? (
            <div className="flex justify-start">
              <p className="  ms-3 text-red-500 text-[13.5px] ">Select a gender</p>
            </div>
          ) : null}
